...
| `supabase/migrations` | Full schema + RLS policies (9 migrations). **Every migration applied against a real local Postgres instance during development** — caught and fixed three genuine bugs along the way: a column-level `REVOKE` that was a silent no-op against Supabase's default table-level `GRANT` (`emergency_access`); client-writable `profiles.email` unbound from `auth.users.email` (secure-sharing identity spoofing, found by a security review); and RLS policies/`SECURITY DEFINER` functions missing explicit `TO authenticated` (found by a `/supabase` skill audit — the function-grant fix itself needed a second pass, since `revoke ... from public` alone didn't touch Supabase's explicit per-role grant to `anon`). Sharing's write-permission fast-follow (0007/0008) landed the same way — a `has_write_share` RLS policy branch, a trigger that caps a write-share recipient to just an item's content/version, and a partial unique index for one-active-share-per-recipient — all re-verified against a fresh local Postgres 16 instance, including the exact scenario 0001_init.sql had flagged as deferred. A follow-up security review of that migration (0009) found and fixed two more low-impact gaps in the same pattern — a policy missing `TO authenticated`, and the trigger's column allowlist missing `created_at` — neither exploitable as written, both fixed anyway. All scenarios — cross-user denial, sharing + revocation, write-permission sharing + its column-restriction trigger, the emergency-access RPC gate, ciphertext-shape constraints, the email-spoof fix, the anon-lockout fix — were exercised against live Postgres, not just written. |
| `apps/web` | Vite/React MVP — signup (with Secret Key reveal), unlock, vault CRUD, password generator, TOTP, Security Dashboard, CSV import, encrypted vault export/import, and **secure item sharing**: multi-recipient (share the same item with any number of people), read-or-write permission per recipient (a write recipient can edit and save an item's content — never its key, which the DB enforces server-side, not just the UI), permission changes and revoke, all X25519. **Builds clean, 15/15 integration tests passing (real crypto round-trips, no mocks — including sharing's full sender→recipient protocol, a write-share recipient's edit path with no VMK involved at all, multi-recipient boxing, and two attack scenarios: a third party can't open someone else's box, and a forged sender identity is rejected), and manually smoke-tested in a live browser** — confirmed `generateSecretKey()`, the strength meter, and the full signup UI work correctly with zero runtime errors. |
...
Emergency access UI, encrypted attachments, passkey support, hardware security keys, broader import formats (1Password/Dashlane/Keeper/NordPass/LastPass), the browser extension's per-site incremental permission flow, and finishing mobile native autofill (fix the iOS extension's `Clibsodium` linking gap, verify the interactive Android fill flow end-to-end, add "save new password" support (`onSaveRequest`) on both platforms). All scoped and sequenced in the build plan §7.

Sharing's write-permission + multi-recipient fast-follow is now built (see the table above) — as is the desktop Quick Access overlay's renderer-side UI (`QuickAccessApp.tsx`), which a stale note here previously still listed as outstanding; both had actually shipped already.
