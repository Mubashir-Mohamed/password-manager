...
| `packages/core-domain` | Zod item schemas, password/passphrase generator, strength scoring. **17/17 tests passing**. |
...
| `apps/web` | Vite/React MVP — signup (with Secret Key reveal), unlock, vault CRUD, password generator, TOTP. **Builds clean, 4/4 integration tests passing (real crypto round-trips, no mocks), and manually smoke-tested in a live browser** — confirmed `generateSecretKey()`, the strength meter, and the full signup UI work correctly with zero runtime errors. |
...
Emergency access UI, encrypted attachments, write-permission sharing, passkey support, hardware security keys, broader import formats, mobile item CRUD/native autofill, the desktop Quick Access overlay's renderer-side UI, and the browser extension's per-site incremental permission flow. All scoped and sequenced in the build plan §7.
